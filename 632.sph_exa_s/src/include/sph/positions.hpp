...
#endif
#ifdef SPEC_OMPSS
#pragma oss taskloop grainsize(TS) in(clist[pi;TS], grad_P_x[pi;TS], grad_P_y[pi;TS], grad_P_z[pi;TS], dt[pi;TS], du[pi;TS]) \
out(vx[pi;TS], vy[pi;TS], vz[pi;TS]) \
inout(x[pi;TS], y[pi;TS], z[pi;TS], u[pi;TS], dt_m1[pi;TS], du_m1[pi;TS])
#endif
    for (pi = 0; pi < n; pi++)
    {
        int i = clist[pi];
...
printf("[DEBUG] Positions computed!\n");
}
} // namespace sph
} // namespace sphexa
